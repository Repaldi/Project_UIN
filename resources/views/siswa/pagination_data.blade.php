...
<div class="row">
  <div class="col-md-12">
    <div class="card" style=" border-radius: 0px 0px 20px 20px; background: white;">
      <!-- <div class="card-header" style=" background: #EDE5E5; border-radius: 0px 0px 0px 0px;">Quiz</div> -->
      <div class="card-body " >
        <?php $i=1; ?>
        @foreach($soal_quiz as $item)

            <div class=" container row" >
                <div class="col-md-3"><h6>Soal No. {{$soal_quiz ->perPage()*($soal_quiz->currentPage()-1)+$i}}   </h6></div>
                <!-- <div class="col-md-8 text-right"><h6>Poin : </h6></div> -->
            </div>
            @if($item->foto != null)
            <div class="container row text-center">
              <img src="{{url('asset-quiz/'.$item->foto)}}" alt="" style="width: 200px;">
            </div>
            @endif
            <div class="container" >
            <table >
                    ...
                    <td>
                        @php
                            $check_jawaban = QuizJawab::where('user_id', Auth::user()->id)
                                    ...
                                    ->where('quiz_siswa_id', $quiz_siswa_id)->first();
                        @endphp
                        @if(!$check_jawaban)
                ...
                <input type="hidden" id="soal_quiz_id" value="{{$item->id}}">
...
<div class="row">
  <div class="col-12 text-center " style=" overflow: Auto;">
    ...
    <input type="hidden" value="{{$quiz_siswa_id}}" id="quiz_siswa_id">
...
$('input[type=radio][name="pilihan"]').click(function() {
    var jawab_quiz = document.querySelector('input[name = "pilihan"]:checked').value;
    var soal_quiz_id    = $("#soal_quiz_id").val();
    var user_id         = $("#user_id").val();
    var quiz_siswa_id   = $('#quiz_siswa_id').val();
    var kunci           = $("#kunci").val();
    ...
    $.ajax({
        url: "{{ url('store/quiz_jawab') }}",
        type: "GET",
        dataType: 'json',
        data: {
            jawab_quiz: jawab_quiz,
            soal_quiz_id: soal_quiz_id,
            user_id: user_id,
            status: status,
